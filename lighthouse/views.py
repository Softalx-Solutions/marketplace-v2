...
class ApprovedDeposits(TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        if request.user.is_authenticated and request.user.is_admin == True:
            ...
            p = Paginator(deposits, 10)
            # page_request_var = 'page'
            page = request.GET.get('page')
            deposit_list = p.get_page(page)
            return render(request, self.template_name, {'deposits':deposits, 'deposit_list':deposit_list})
        else:
            messages.error(request, 'You do not have permission to access this page')
            return redirect('login')
...
class ApprovedWithdrawals(TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        if request.user.is_authenticated and request.user.is_admin == True:
            ...
            p = Paginator(withdrawals, 10)
            # page_request_var = 'page'
            page = request.GET.get('page')
            withdrawal_list = p.get_page(page)
            return render(request, self.template_name, {'withdrawals':withdrawals, 'withdrawal_list':withdrawal_list})
        else:
            messages.error(request, 'You do not have permission to access this page')
            return redirect('login')
